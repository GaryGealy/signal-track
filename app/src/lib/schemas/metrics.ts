...
export const weightSchema = z.object({
	...
	value: z.coerce.number().refine((val) => val > 0, { message: 'Weight must be positive' }),
...
export const bloodPressureSchema = z.object({
	...
	systolic: z.coerce.number().int().min(1, 'Required'),
	diastolic: z.coerce.number().int().min(1, 'Required'),
	date: z.string().min(1, 'Date is required'),
	time: z.string().min(1, 'Time is required')
});
...
export const sleepSchema = z
	...
	.refine((d) => new Date(`${d.wakeDate}T${d.wakeTime}`) > new Date(`${d.bedDate}T${d.bedTime}`), {
...
export const workSchema = z
	...
	.refine(
		(d) => new Date(`${d.workDate}T${d.endTime}`) > new Date(`${d.workDate}T${d.startTime}`),
